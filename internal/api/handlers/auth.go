...
func (h *Handler) FacebookLoginHandler(c *gin.Context) {
	...
	if appID != nil && appSecret != nil {
		fbConfig = authhelp.GenerateFacebookConfig(appID.(string), appSecret.(string), h.Config.DomainName+"/auth/facebook/callback")
	} else {
...
func (h *Handler) FacebookRefreshTokenHandler(c *gin.Context) {
	...
	fbConfig := authhelp.GenerateFacebookConfig(appID, appSecret, h.Config.DomainName+"/auth/facebook/callback")
